...
def print_coherence(model='cr_gsm', url='./data/Snippets/train.feat', vocab_size=30642):
...
if __name__ == '__main__':
    ...
    with codecs.open('./StackOverflow/gsm_train_theta', 'rb') as fp:
    # with codecs.open('./Snippets/cr_gsm_train_theta', 'rb') as fp:
        theta = pickle.load(fp)
    fp.close()

    with codecs.open('./StackOverflow/gsm_train_beta', 'rb') as fp:
    # with codecs.open('./Snippets/cr_gsm_train_beta', 'rb') as fp:
        beta = pickle.load(fp)
    fp.close()

    tw = get_top_words(beta, vocab, 15)

    test_mat = data_set('./data/Snippets/train.feat', 30642)
